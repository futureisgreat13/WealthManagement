...
if active_debt:
    val_rows = []
    col_source_map = {}
    year_strs = [str(yr) for yr in val_years]

    # Initialize col_source_map for each year column
    for yr_str in year_strs:
        col_source_map[yr_str] = {}

    for row_idx, d in enumerate(active_debt):
        row = {"Name": d.get("name", "")}
        ...
        for yr in val_years:
            ...
            if yr_str in vh:
                ...
                col_source_map[yr_str][row_idx] = "input"
            else:
    ...
    proj_df = pd.DataFrame(val_rows)
    bg_style_map, cell_style_map = utils.build_valuation_style_maps(col_source_map)
    formula_map = utils.get_formula_map("debt_valuations", len(proj_df), year_strs)

    grid_result = utils.render_editable_aggrid_table(
        proj_df,
        key="aggrid_debt_valuations",
        height=min(400, 60 + len(proj_df) * 35),
        editable_cols=year_strs,
        numeric_cols=year_strs,
    ...
    edited_val = grid_result.data
    edited_val = utils.process_math_in_df(edited_val, year_strs, editor_key="debt_valuations")
