...
# --- Positions (editable) ---
st.subheader(f"Positions ({len(items)})")
edit_rows = [{
    "name": p.get("name", ""),
...
st.markdown('<p style="background:#1b4332;color:#a7f3d0;padding:4px 12px;border-radius:4px;font-size:0.85em;margin:0">✏️ Editable — add/edit positions directly. Supports math (e.g. =500*2, 1000/EURUSD)</p>', unsafe_allow_html=True)
...
st.caption("🟡 Yellow = user-entered actual. Default = formula (IRR × probability). Double-click to edit.")
...
formula_map = utils.get_formula_map("private_equity_valuations", len(proj_df), year_strs)

grid_result = utils.render_editable_aggrid_table(
    proj_df, key="pe_valuation_aggrid",
    editable_cols=year_strs,
    numeric_cols=["Current"] + year_strs,
    bg_style_map=bg_style_map, cell_style_map=cell_style_map,
    formula_map=formula_map, editor_key="private_equity_valuations",
